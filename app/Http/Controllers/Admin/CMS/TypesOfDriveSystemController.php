...
class TypesOfDriveSystemController extends Controller
{
    /**
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('image', function ($row) {
                    if ($row->image) {
                        $src = $row->image;
                        $image = '<img src="' . $src . '" alt="Types of Drive System Image" class="td_img_50">';
                    } else {
                        $image = '<img src="' . asset('admin/img/shop-img/no_image.png') . '" alt="Types of Drive System Image" class="td_img_50">';
                    }
                    return $image;
                })
                ->addColumn('action', function ($row) {
                    $btn = '<div class="d-flex align-items-center gap-3">
                        <a href="' . url('cms/knowledgebases/type-of-drive-system-edit/' . $row->id) . '">
    ...
    public function store(Request $request)
    {
        ...
        $isInserted = TypesOfDriveSystem::insert([
            'title' => $request->title,
        ...
        if ($isInserted) {
            return redirect('cms/knowledgebases/type-of-drive-system-list')->with('success_msg', 'Data added successfully!');
        } else {
            return redirect('cms/knowledgebases/type-of-drive-system-list')->with('error_msg', 'Something went wrong!');
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        $existingData = TypesOfDriveSystem::where('id', $id)->first();
        if (!$existingData) {
            return redirect('cms/knowledgebases/type-of-drive-system-list')->with('error_msg', 'Data not found.');
        }
        ...
        // Perform the update
        $isUpdated = TypesOfDriveSystem::where('id', $id)->update($updateData);

        if ($isUpdated) {
            return redirect('cms/knowledgebases/type-of-drive-system-list')->with('success_msg', 'Data updated successfully!');
        } else {
            return redirect('cms/knowledgebases/type-of-drive-system-list')->with('error_msg', 'Failed to update data.');
        }
    }
    ...
    {
        $result = TypesOfDriveSystem::where('id', $request->id)->first();
        ...
        if ($result) {
            ...
            TypesOfDriveSystem::where('id', $request->id)->delete();
